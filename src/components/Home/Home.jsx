...
import Pies from './sections/Pies';
...
const Home = () => {
  ...
  const [data, setData] = useState(null); // Add a state for your data
  ...
  useEffect(() => {
    ...
    if (data === null) {
      const fetchData = async () => {
        setIsLoading(true);
        await new Promise((resolve) => setTimeout(resolve, 1000)); // Simulate a data fetching function
        const newData = 'Your fetched data'; // Replace this with your actual data fetching logic
        setData(newData);
        setIsLoading(false);
      };
